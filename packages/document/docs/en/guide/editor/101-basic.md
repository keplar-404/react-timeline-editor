...
> Note: All code examples use inline `<style>` CSS to be framework-agnostic.

## Standard Timeline Editor

Try dragging the action block to the right; it automatically recognizes infinite scrolling when dragged near the edge.
...
Hide the timeline cursor line by setting `hideCursor` to `true`.

```tsx
<Timeline hideCursor={true} editorData={data} effects={mockEffect} />
```

## Using a Ref for Imperative Control

Attach a `ref` to access the `TimelineState` handle, which lets you programmatically control playback.

```tsx
import React, { useRef } from 'react';
import { Timeline, TimelineState } from '@keplar-404/react-timeline-editor';

export const EditorWithRef = () => {
  ...
  return (
    <>
      ...
      <button onClick={() => timelineRef.current?.setTime(0)}>⏮ Reset</button>
